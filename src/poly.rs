...
const UNIFORM_GAMMA1_NBLOCKS: usize = (params::POLYZ_PACKEDBYTES + fips202::SHAKE256_RATE - 1) / fips202::SHAKE256_RATE;
...
pub fn challenge(c: &mut Poly, seed: &[u8]) {
    let mut state = fips202::KeccakState::default();
    fips202::shake256_absorb(&mut state, seed, params::SEED_BYTES);
...
pub fn eta_pack(r: &mut [u8], a: &Poly) {
    let mut t = [0u8; 2];
    for i in 0..N / 2 {
        t[0] = (params::ETA as i32 - a.coeffs[2 * i + 0]) as u8;
        t[1] = (params::ETA as i32 - a.coeffs[2 * i + 1]) as u8;
        r[i] = t[0] | (t[1] << 4);
    }
}
...
pub fn eta_unpack(r: &mut Poly, a: &[u8]) {
    for i in 0..N / 2 {
        ...
        r.coeffs[2 * i + 0] = params::ETA as i32 - r.coeffs[2 * i + 0];
        r.coeffs[2 * i + 1] = params::ETA as i32 - r.coeffs[2 * i + 1];
    }
}
